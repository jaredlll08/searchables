...
/**
 * Applies style formatting for an {@link net.minecraft.client.gui.widget.TextFieldWidget}, intended to be passed into {@link net.minecraft.client.gui.widget.TextFieldWidget#setFormatter(BiFunction)}.
 */
public class FormattingVisitor implements ContextAwareVisitor<TokenRange, FormattingContext>, Consumer<String>, BiFunction<String, Integer, IReorderingProcessor> {
    ...
    @Override
    public IReorderingProcessor apply(final String currentString, final Integer offset) {
        
        List<IReorderingProcessor> sequences = new ArrayList<>();
        int index = 0;
        for(Pair<TokenRange, Style> token : tokens) {
            ...
            if(subStart > 0) {
                sequences.add(IReorderingProcessor.forward(currentString.substring(index, subEnd), token.getSecond()));
                sequences.add(IReorderingProcessor.forward(currentString.substring(subEnd, subStart), token.getSecond()));
                index = subStart;
            }
        }
        sequences.add(IReorderingProcessor.forward(currentString.substring(index), Style.EMPTY));
        
        return IReorderingProcessor.composite(sequences);
    }
