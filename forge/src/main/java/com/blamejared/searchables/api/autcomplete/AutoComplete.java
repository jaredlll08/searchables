 ...
 */
public class AutoComplete<T> extends Widget implements Consumer<String> {
    ...
    public AutoComplete(SearchableType<T> type, AutoCompletingEditBox<T> editBox, Supplier<List<T>> entries, int x, int y, int width, int suggestionHeight, int maxSuggestions) {
        ...
        super(x, y, width, suggestionHeight * maxSuggestions, new StringTextComponent(""));
        this.type = type;
    ...
    public boolean mouseScrolled(double xpos, double ypos, double delta) {
        ...
        if(isMouseOver(xpos, ypos) || this.editBox().isMouseOver(xpos, ypos)) {
            displayOffset = (int) MathHelper.clamp(displayOffset - delta, 0, Math.max(this.suggestions.size() - maxSuggestions, 0));
    ...
    @Override
    public void renderButton(MatrixStack pose, int mx, int my, float partial) {
        ...
        for(int i = displayOffset; i < Math.min(displayOffset + maxSuggestions, suggestions.size()); i++) {
            ...
            Minecraft.getInstance().font.drawShadow(pose, suggestion.display(), minX, minY + 1, hovered ? Objects.requireNonNull(TextFormatting.YELLOW.getColor()) : 0xFFFFFFFF);
        }
    ...
    private void offsetDisplay(int offset) {
        ...
        offset = MathHelper.clamp(offset, 0, shownSuggestions() - 1);
